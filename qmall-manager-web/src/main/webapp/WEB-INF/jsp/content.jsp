    ...
    handler:function(){
		...
		E3.createWindow({
			...
			onLoad : function(){
				...
				//Q:为了防止2次加载的情况,需要清除上一次加载的图片
				$("#contentEditForm [name=pic]").next().not(".easyui-linkbutton").remove();
				$("#contentEditForm [name=pic2]").next().not(".easyui-linkbutton").remove();
				// 实现图片
				if(data.pic){
    	    ...
    	    if (r){
            	...
            	$.post("/content/delete",params, function(data){
        			if(data.status == 200){
        				...
        				$.messager.alert('提示','删除内容失败!','error');
        			}
        		});
    	    }
    	});
